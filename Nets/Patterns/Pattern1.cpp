...
int Pattern1::internodal_data_list_indexer(int node_ID, int layer_ID, int connection_ID)
{
    int IDX = (node_ID*architecture[layer_ID+1]) + connection_ID;
    int iter = 0;
start:
    if (iter == (layer_ID))
    {
    ...
    {   
        IDX += architecture[iter]*architecture[iter+1];
        iter++;
        goto start;
    }
}
...
start:
    ...
    else
    {
        ...
        {
            int IDX = internodal_data_list_indexer(node, layer, connectionIDX);
